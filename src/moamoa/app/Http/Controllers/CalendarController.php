...
class CalendarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $dailyIncomeData = Mission::selectRaw('missions.updated_at as target_at, SUM(amount) as income') // start_at -> updated_at
                                ->whereBetween('missions.updated_at', [$startDate, $endDate]) // updated_at 사용
                                ->groupBy('missions.updated_at') // updated_at 기준으로 그룹화
                                ->orderBy('missions.updated_at') // updated_at 기준으로 정렬
                                ->get();

        // 일별 전체 지출 합산 (updated_at 기준)
        $dailyOutgoData = Transaction::selectRaw('transactions.updated_at as target_at, SUM(amount) as income') // transaction_date -> updated_at
                                    ->whereBetween('transactions.updated_at', [$startDate, $endDate]) // updated_at 사용
                                    ->groupBy('transactions.updated_at') // updated_at 기준으로 그룹화
                                    ->orderBy('transactions.updated_at') // updated_at 기준으로 정렬
                                    ->get();
